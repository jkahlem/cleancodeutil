...
func UnmarshalMethod(records [][]string) []Method {
	...
	for i, record := range records {
		result[i].ClassName = record[0]
		result[i].ReturnType = record[1]
		result[i].MethodName = record[2]
		result[i].Parameters = SplitList(record[3])
		result[i].Labels = SplitList(record[4])
		result[i].Modifier = SplitList(record[5])
		result[i].ClassField = record[6]
		result[i].FilePath = record[7]
	}
	return result
}
...
func UnmarshalClass(records [][]string) []Class {
	...
	for i, record := range records {
		result[i].ClassName = record[0]
		result[i].Extends = SplitList(record[1])
	}
	return result
}
...
func UnmarshalTypeConversion(records [][]string) []TypeConversion {
	...
	for i, record := range records {
		result[i].SourceType = record[0]
		result[i].DestinationType = record[1]
	}
	return result
}
...
func UnmarshalMethodGenerationDatasetRow(records [][]string) []MethodGenerationDatasetRow {
	...
	for i, record := range records {
		result[i].ClassName = record[0]
		result[i].MethodName = record[1]
		result[i].Parameters = SplitList(record[2])
	}
	return result
}
...
func UnmarshalIdealResult(records [][]string) []IdealResult {
	...
	for i, record := range records {
		result[i].FilePath = record[0]
		result[i].FileType = record[1]
		result[i].Identifier = record[2]
		result[i].IdentifierType = record[3]
		...
		result[i].IssueID = record[6]
		result[i].IssueAdditionalDetail = record[7]
		result[i].IssueCategory = record[8]
		result[i].IssueDetail = record[9]
		result[i].AnalysisDateTime = record[10]
	}
	return result
}
