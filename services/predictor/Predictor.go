...
type Predictor interface {
	PredictToMap(mapping MethodTypeMap) errors.Error
	Predict(methodNames []PredictableMethodName) ([]string, errors.Error)
	Train(labels, trainingSet, evaluationSet [][]string) (Evaluation, errors.Error)
}

type predictor struct{}

// Makes predictions for the methods in the map and sets the types as their value.
func (p *predictor) PredictToMap(mapping MethodTypeMap) errors.Error {
	names := p.getMethodNamesInsideOfMap(mapping)
	predictedTypes, err := p.Predict(names)
	if err != nil {
		return err
	}
...
func (p *predictor) Predict(methodNames []PredictableMethodName) ([]string, errors.Error) {
	strSlice := make([]string, len(methodNames))
	for i, name := range methodNames {
		strSlice[i] = string(name)
	}
	return getInterface().ProxyFacade().Predict(strSlice)
}

// Starts the training and evaluation process. Returns the evaluation result if finished.
func (p *predictor) Train(labels, trainingSet, evaluationSet [][]string) (Evaluation, errors.Error) {
	return getInterface().ProxyFacade().Train(p.asCsvString(labels), p.asCsvString(trainingSet), p.asCsvString(evaluationSet))
}
